...
if(twEl&&!R){var twHTML=twEl.innerHTML;var twChars=twHTML.replace(/<br\s*\/?>/gi,'\n').split('');twEl.innerHTML='';var twGhost=document.createElement('span');twGhost.className='tw-ghost';twGhost.innerHTML=twHTML;var twLive=document.createElement('span');twLive.className='tw-live';twLive.setAttribute('aria-hidden','true');twChars.forEach(function(ch){if(ch==='\n'){twLive.appendChild(document.createElement('br'));return}var sp=document.createElement('span');sp.className='tw-c';sp.textContent=ch;twLive.appendChild(sp)});var twCaret=document.createElement('i');twCaret.className='tw-caret';twLive.appendChild(twCaret);twEl.appendChild(twGhost);twEl.appendChild(twLive);var twCells=twLive.querySelectorAll('.tw-c');var twPos=[];for(var twI=0;twI<twCells.length;twI++){twPos.push('translate('+(twCells[twI].offsetLeft-7)+'px,'+(twCells[twI].offsetTop+Math.round(twCells[twI].offsetHeight*.18))+'px)')}var twIdx=0;function twType(){if(twIdx<twCells.length){var c=twCells[twIdx];c.classList.add('tw-on');twCaret.style.transform=twPos[twIdx];twIdx++;var d=c.textContent==='.'?400:65+Math.random()*45;setTimeout(twType,d)}else{setTimeout(function(){twEl.innerHTML=twHTML},1600)}}setTimeout(twType,800)}
...
var fontStep=0;var maxFont=3;var minFont=-1;
...
function updateToggles(){
  ...
  panel.querySelectorAll('[data-a11y]').forEach(function(btn){
    ...
    if(key==='font-increase'||key==='font-decrease'||key==='font-reset'||key==='reset')return;
...
function changeFontSize(dir){
  ...
  if(dir==='increase'&&step<maxFont)step++;
  else if(dir==='decrease'&&step>minFont)step--;
