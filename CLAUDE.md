...
### 02/08/2026 — שחזור סקשן ההוכחה עם התמונות
- **מקור הרגרסיה אותר**: commit `263de41` (07/05/2026), שכותרתו תיקון Schema, החזיר את `index.html` לגרסה ישנה (118 הוספות מול **226 מחיקות**) והפיל איתו את סקשן ההוכחה חזרה ל-`grid three` של כרטיסיות אייקון בלבד. שלוש התמונות שנוספו ב-`ac5f3ef` אבדו בשוגג ולא בכוונה. אחר כך `0ef0268` ניקה אותן גם מהסייטמאפ — בצדק, כי באותו רגע הן באמת היו יתומות.
- **השחזור**: הסימון `proof-cards` / `card--with-image` נשלף מ-`263de41^` (המצב רגע לפני האובדן) והוחזר כלשונו, כולל `<picture>` עם 3 גדלים × 2 פורמטים, `alt` מפורט, `loading="lazy"` ו-`width`/`height` מפורשים נגד CLS. אומת אוטומטית שהטקסט (h3 + p) **זהה** בין הגרסאות — השחזור לא שינה אף מילת תוכן.
- ה-CSS של הפריסה האדיטוריאלית (`.proof-cards`, `.card--with-image`, כולל תיקון ה-220px ל-iOS Safari מ-03/05) **שרד את הרגרסיה במלואו** — רק ה-HTML איבד את התמונות. לא נדרש שינוי עיצוב.
- 3 רשומות התמונה הוחזרו ל-`sitemap.xml` (7 → 10 תמונות).
- **🐛 באג נלווה שתוקן**: `40f03fe` הפך את כפתור הוואטסאפ הצף לעיגול-אייקון והסיר את הכלל `.wa-float__label{white-space:nowrap}`, אבל **השאיר את ה-`<span>` ב-HTML**. מאז הטקסט "דברו איתי" נשפך מחוץ לעיגול של 48px בשני דפים. ה-span הוסר; ה-`aria-label` על הקישור נשאר ומספק את השם הנגיש.
- אומת מחדש אחרי השחזור: 11/11 תמונות נטענות (דסקטופ ומובייל), 0 כשלי ניגודיות, 0 אלמנטים מוסתרים בלי JS, אפס גלישה אופקית ב-320/390/768.
