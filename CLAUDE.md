...
## היסטוריית שינויים
### 02/08/2026 — יישום `web-build-standard`: תיקוני נגישות ועמידות קריטיים
כל הממצאים נמדדו בדפדפן (Playwright + Chromium), לא הוערכו.

- **🐛 הדף היה ריק לחלוטין בלי JavaScript.** `.reveal`, `.reveal-stagger>*`, `.hero-copy>*` ו-`.hero-visual` הוגדרו `opacity:0` ב-CSS ונחשפו רק ע"י JS — 76 מתוך 76 האלמנטים הנמדדים היו שקופים. תוקן ע"י גידור כל כללי ההסתרה תחת מחלקת `.js` שנוספת ב-inline script ב-`<head>` לפני ה-paint הראשון.
- **🐛 תפריט הנגישות מת אצל מי שהפעיל `prefers-reduced-motion`.** `main.js` פתח ב-`if(R)return;` בשורה 30, והווידג'ט ישב בשורה 123 — כלומר בדיוק המשתמשים שהכי צריכים אותו איבדו אותו. יחד איתו נפלו גם כפתור הוואטסאפ הצף וכפתור חזרה-למעלה (שניהם `opacity:0` + `pointer-events:none` עד ש-JS חושף אותם). המשמר שוכתב ל-`if(!R){...}` שעוטף **רק** תנועה דקורטיבית; נגישות, כפתורים צפים ומצב ה-header רצים תמיד.
- **🐛 כפתור הוואטסאפ בדף המאמר היה בלתי נראה לצמיתות** — הדף לא טען JS כלל, ולכן `.visible` מעולם לא נוסף. ברירת המחדל הופכה: `.wa-float` גלוי, ורק `.js .wa-float--reveal` (דף הבית) מתחיל מוסתר.
- **נגישות בכל דף (ת"י 5568)**: דף המאמרים, דף המאמר ו-404 היו **בלי כפתור נגישות ובלי הצהרת נגישות**. נוצר `a11y.js` עצמאי שמזריק את הווידג'ט — מקור אמת יחיד במקום 4 עותקים; הסימון הידני הוסר מ-`index.html`. בנוסף נוסף קישור גלוי להצהרת הנגישות בפוטר של הדפים המשניים, שעובד גם בלי JS.
- **מלכודת 6 (`IntersectionObserver` ל-scroll reveal)**: הוחלף בבדיקת מיקום ב-scroll handler. IO מפספס אלמנטים בקפיצת עוגן; אומת שכניסה ל-`#faq` (scrollY≈8000) משאירה 0 סקשנים בלתי נראים.
- **ניגודיות — 0 כשלים בכל 4 הדפים** (היו 4). השורש: `--primary-strong` (`#b8941f`) כטקסט על המשטחים הבהירים = 2.69:1, מתחת לרף 4.5:1. נוסף טוקן `--primary-ink: #8a6a1c` (4.72:1 על קרם, 5.05:1 על לבן) והוחל על 5 נקודות. שים לב: `.text-review cite` יושב על רקע **כהה** — שם `--primary-ink` היה מחמיר ל-3.65:1, ולכן הוחזר ל-`--primary-strong` (6.40:1).
- **אזורי לחיצה**: קישורי הניווט היו בגובה 17px. נוסף `min-height:24px` + padding לתפריט, לפוטר, לפירורי הלחם ולקישור החזרה. נותרו קישורי טקסט בתוך משפט (inline) — פטורים מפורשות לפי WCAG 2.2 Target Size.
- **גלילה אופקית ב-320px** (היה 330px רוחב מסמך מול 320px viewport). שני מקורות: `.footer-links` בלי `flex-wrap`, ו-`.skip-link` שחנה ב-`inset-inline-start:-9999px` — ב-RTL זה נופל מחוץ לקצה **הימני** ומגדיל את שטח הגלילה. הקישור הועבר לחניה אנכית (`top:-100px`), שאינה תלוית-כיווניות. אומת 320/390/768 בכל 4 הדפים.
- **טיפוגרפיה עברית**: הוסר `letter-spacing` חיובי מ-3 אלמנטים בעברית טהורה (`.eyebrow`, `.article-eyebrow`, `.article-card__eyebrow`); tracking בכותרות הודק לטווח התקן (-.03em); נוספו `overflow-wrap:break-word` ו-`line-height` ל-h2/h3; מספרי הטלפון (8 מופעים) נעטפו ב-`<bdi>`.
- **`scripts/build_assets.sh`** — build אמיתי מבוסס esbuild ל-`style.min.css`/`main.min.js`/`a11y.min.js`, עם בדיקת NaN ואיזון סוגריים. מחליף את המינון הידני שבעבר שלח `animation-duration:NaNs` לבלוק `prefers-reduced-motion`.
- cache-busting: כל הנכסים ל-`?v=20260802`; `lastmod` בסייטמאפ ל-2026-08-02.
- **לא טופל (דורש החלטת תוכן)**: 18 קבצי `proof-*.jpg/webp` יושבים בריפו אך **אינם מקושרים משום מקום** — סקשן ההוכחה מוצג היום בלי התמונות שתועדו ב-03/05/2026.
